...
- **Иконки фона таба** — реальные квадранты из клиента (см. раздел "Иконки" выше), НЕ образы, хотлинкнутые/скопированные с вовхеда. `tabBackgroundStyle(tab)` строит `backgroundImage`/`backgroundPosition` из 4 URL API-ответа; поверх них — CSS `::before` с радиальным градиентом + `inset box-shadow` (тёмная виньетка, чтобы иконки талантов не терялись на фоне яркого арта).
